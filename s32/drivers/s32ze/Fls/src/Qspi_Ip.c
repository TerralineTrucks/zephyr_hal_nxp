...
/* Check if current file and Qspi_Ip_Controller header file are of the same
 * Autosar version */
#if ((QSPI_IP_AR_RELEASE_MAJOR_VERSION_C !=             \
     ...
     (QSPI_IP_AR_RELEASE_REVISION_VERSION_C !=          \
      QSPI_IP_CONTROLLER_AR_RELEASE_REVISION_VERSION_H))
#error \
    "AutoSar Version Numbers of Qspi_Ip.c and Qspi_Ip_Controller.h are different"
#endif
/* Check if current file and Qspi_Ip_Controller header file are of the same
 * Software version */
...
#error \
    "Software Version Numbers of Qspi_Ip.c and Qspi_Ip_Controller.h are different"
#endif
...
#endif
/* Check if current file and Qspi_Ip_Common header file are of the same Autosar
 * version */
#if ((QSPI_IP_AR_RELEASE_MAJOR_VERSION_C !=         \
      QSPI_IP_COMMON_AR_RELEASE_MAJOR_VERSION_H) || \
     (QSPI_IP_AR_RELEASE_MINOR_VERSION_C !=         \
      QSPI_IP_COMMON_AR_RELEASE_MINOR_VERSION_H) || \
     (QSPI_IP_AR_RELEASE_REVISION_VERSION_C !=      \
      QSPI_IP_COMMON_AR_RELEASE_REVISION_VERSION_H))
#error "AutoSar Version Numbers of Qspi_Ip.c and Qspi_Ip_Common.h are different"
#endif
/* Check if current file and Qspi_Ip_Common header file are of the same Software
 * version */
...
#error \
    "Software Version Numbers of Qspi_Ip.c and Qspi_Ip_Common.h are different"
#endif
...
/* Check if current file and Qspi_Ip_Hyperflash header file are of the same
 * vendor */
 ...
 * Autosar version */
#if ((QSPI_IP_AR_RELEASE_MAJOR_VERSION_C !=             \
      QSPI_IP_HYPERFLASH_AR_RELEASE_MAJOR_VERSION_H) || \
     (QSPI_IP_AR_RELEASE_MINOR_VERSION_C !=             \
      QSPI_IP_HYPERFLASH_AR_RELEASE_MINOR_VERSION_H) || \
     (QSPI_IP_AR_RELEASE_REVISION_VERSION_C !=          \
      QSPI_IP_HYPERFLASH_AR_RELEASE_REVISION_VERSION_H))
#error \
    "AutoSar Version Numbers of Qspi_Ip.c and Qspi_Ip_Hyperflash.h are different"
#endif
/* Check if current file and Qspi_Ip_Hyperflash header file are of the same
 * Software version */
...
#error \
    "Software Version Numbers of Qspi_Ip.c and Qspi_Ip_Hyperflash.h are different"
#endif
...
static Qspi_Ip_StatusType Qspi_Ip_InitReset(uint32 instance, uint16 resetCmdLut,
                                            uint8 resetCmdCount,
                                            const Qspi_Ip_StateType *state);

static Qspi_Ip_StatusType Qspi_Ip_InitWriteReg(
    uint32 instance, const Qspi_Ip_InitOperationType *operation);

static Qspi_Ip_StatusType Qspi_Ip_InitRMWReg(
    uint32 instance, const Qspi_Ip_InitOperationType *operation);

static Qspi_Ip_StatusType Qspi_Ip_InitReadReg(
    uint32 instance, const Qspi_Ip_InitOperationType *operation);

static Qspi_Ip_StatusType Qspi_Ip_InitProtection(
    uint32 instance, const Qspi_Ip_StateType *state);
...
static uint16 Qspi_Ip_InitLutSeq(uint32 instance, uint16 VirtualLut,
                                 uint8 LutRegister);

/*FUNCTION**********************************************************************
 *
 * Function Name : Qspi_Ip_InitLutSeq
 * Description   : Initializes one sequence in the LUT table from a virtual
 * table sequence. Returns start index of next sequence.
 * @implements      Qspi_Ip_InitLutSeq_Activity */
static uint16 Qspi_Ip_InitLutSeq(uint32 instance, uint16 VirtualLut,
                                 uint8 LutRegister) {
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  const Qspi_Ip_InstrOpType *virtualLutTable =
      state->configuration->lutSequences.lutOps;
  Qspi_Ip_InstrOpType operation1, operation2;
  uint16 vLutIdx = VirtualLut;                    /* Index in virtual LUT  */
  uint8 lutIdx = 0U;                              /* Index in physical LUT */
  uint32 lutData[FEATURE_QSPI_LUT_SEQUENCE_SIZE]; /* Data to be written to the
                                                     physical LUTs */
  ...
  do {
    ...
    lutData[lutIdx] = QSPI_IP_PACK_LUT_REG(operation1, operation2);
    lutIdx++;
  ...
  */
  for (; lutIdx < FEATURE_QSPI_LUT_SEQUENCE_SIZE; lutIdx++) {
    lutData[lutIdx] =
  ...
  Qspi_Ip_WriteLuts(state->connection->qspiInstance,
                    FEATURE_QSPI_LUT_SEQUENCE_SIZE * LutRegister, lutData,
                    lutIdx);
...
static void Qspi_Ip_SetBitfield(uint8 *data, uint8 size, uint8 shift,
                                uint8 width, uint32 value) {
  uint8 cnt;
  uint32 longData = 0UL;
  uint32 mask;
...
static uint32 Qspi_Ip_GetBitfield(const uint8 *data, uint8 size, uint8 shift,
                                  uint8 width) {
  uint8 cnt;
  uint32 longData = 0U;
  uint32 mask;
...
static Qspi_Ip_StatusType Qspi_Ip_UpdateStatusReg(uint32 instance, uint8 offset,
                                                  uint8 width, uint8 value) {
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  const Qspi_Ip_StatusConfigType *statusConfig =
      &(state->configuration->statusConfig);
  uint8 data[4U];
  Qspi_Ip_StatusType status;

  /* Read status register */
  status = Qspi_Ip_RunReadCommand(instance, statusConfig->statusRegReadLut, 0U,
                                  data, NULL_PTR, statusConfig->regSize);

  if (STATUS_QSPI_IP_SUCCESS == status) {
    /* Check existing value, write status register only if needed */
    if (value !=
        Qspi_Ip_GetBitfield(data, statusConfig->regSize, offset, width)) {
    ...
    if (STATUS_QSPI_IP_SUCCESS == status) {
      ...
      status =
          Qspi_Ip_RunWriteCommand(instance, statusConfig->statusRegWriteLut, 0U,
                                  data, statusConfig->regSize);
    }
  }
...
static Qspi_Ip_StatusType Qspi_Ip_CheckStatusReg(uint32 instance, uint8 offset,
                                                 uint8 width, uint8 *value) {
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  const Qspi_Ip_StatusConfigType *statusConfig =
      &(state->configuration->statusConfig);
  ...
  if (STATUS_QSPI_IP_SUCCESS == status) {
    /* Extract bit-field */
    *value =
        (uint8)Qspi_Ip_GetBitfield(data, statusConfig->regSize, offset, width);
  }
 ...
 * Description   : Enables the serial flash memory for a program or erase
 * operation
...
static Qspi_Ip_StatusType Qspi_Ip_WriteEnable(uint32 instance) {
  ...
  while (retries > 0UL) {
    ...
    /* check WEL bit */
    cmdStatus = Qspi_Ip_CheckStatusReg(
        instance, statusConfig->writeEnableOffset, 1U, &welValue);
...
static Qspi_Ip_StatusType Qspi_Ip_BasicErase(uint32 instance, uint32 address,
                                             uint16 eraseLut) {
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_ERROR;

  /* Check device type */
  if (QSPI_IP_SERIAL_FLASH == state->configuration->memType) {
    status = Qspi_Ip_SerialflashSectorErase(instance, address, eraseLut);
  } else /*QSPI_IP_HYPER_FLASH == state->configuration->memType */
  {
...
static Qspi_Ip_StatusType Qspi_Ip_CheckMemoryStatus(uint32 instance,
  ...
  if (STATUS_QSPI_IP_SUCCESS == status) {
    ...
    /* Read status register */
    status = Qspi_Ip_RunReadCommand(instance, VirtualLut, 0U, data, NULL_PTR,
                                    statusConfig->regSize);
    if (STATUS_QSPI_IP_SUCCESS == status) {
      /* Extract bit-field */
      busyValue = (uint8)Qspi_Ip_GetBitfield(data, statusConfig->regSize,
                                             statusConfig->busyOffset, 1U);
...
static Qspi_Ip_StatusType Qspi_Ip_CheckCommandComplete(uint32 instance,
  ...
  u32TimeoutTicks = OsIf_MicrosToTicks(QSPI_IP_FLS_INIT_TIMEOUT,
 ...
 * Description   : Wait until external memory is available for operation after a
 * reset
...
static void Qspi_Ip_CheckResetComplete(void) {
  ...
  /* Prepare timeout counter */
  u32TimeoutTicks = OsIf_MicrosToTicks(QSPI_IP_RESET_TIMEOUT,
                                       (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
  u32CurrentTicks = OsIf_GetCounter((OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
 ...
 * Description   : Write the configured value into a register of external flash
 * device
...
static Qspi_Ip_StatusType Qspi_Ip_InitWriteReg(
  ...
  if (STATUS_QSPI_IP_SUCCESS == status) {
    ...
    status = Qspi_Ip_RunWriteCommand(instance, operation->command1Lut,
                                     operation->addr, value, operation->size);
  }
...
static Qspi_Ip_StatusType Qspi_Ip_InitRMWReg(
  ...
  status =
      Qspi_Ip_RunReadCommand(instance, operation->command1Lut, operation->addr,
                             value, NULL_PTR, operation->size);
  if (STATUS_QSPI_IP_SUCCESS == status) {
    /* Retrieve target bitfield */
    fieldVal = Qspi_Ip_GetBitfield(value, operation->size, operation->shift,
                                   operation->width);
    if (fieldVal != operation->value) {
      /* Modify target bitfield */
      Qspi_Ip_SetBitfield(value, operation->size, operation->shift,
                          operation->width, operation->value);
      if (QSPI_IP_LUT_INVALID != operation->weLut) {
        /* Send WREN command */
        status =
            Qspi_Ip_RunCommand(instance, operation->weLut, operation->addr);
      }
      if (STATUS_QSPI_IP_SUCCESS == status) {
        /* Write back register value; use second LUT command */
        status = Qspi_Ip_RunWriteCommand(instance, operation->command2Lut,
                                         operation->addr, (uint8 *)&value,
                                         operation->size);
      }
    }
  }
 ...
 * Description   : Read the register's value of external flash device and loop
 * until matching the configured one
...
static Qspi_Ip_StatusType Qspi_Ip_InitReadReg(
  ...
  do {
    /* read current register value */
    status = Qspi_Ip_RunReadCommand(instance, operation->command1Lut,
                                    operation->addr, value, NULL_PTR,
                                    operation->size);
    if (STATUS_QSPI_IP_SUCCESS == status) {
      /* retrieve target bitfield */
      fieldVal = Qspi_Ip_GetBitfield(value, operation->size, operation->shift,
 ...
 * Description   : Execute initialization sequence to get the serial flash
 * memory in the target state for operation */
static Qspi_Ip_StatusType Qspi_Ip_InitOperation(uint32 instance,
                                                const Qspi_Ip_StateType *state,
                                                uint8 initOp) {
  ...
  switch (initOperations[initOp].opType) {
    case QSPI_IP_OP_TYPE_CMD:
      ...
      status = Qspi_Ip_RunCommand(instance, initOperations[initOp].command1Lut,
                                  initOperations[initOp].addr);
      break;
    ...
    case QSPI_IP_OP_TYPE_QSPI_CFG:
      ...
      status = Qspi_Ip_ControllerInit(state->connection->qspiInstance,
                                      initOperations[initOp].ctrlCfgPtr);
      break;
...
static Qspi_Ip_StatusType Qspi_Ip_InitProtection(
    uint32 instance, const Qspi_Ip_StateType *state) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  uint8 configProtection =
      state->configuration->statusConfig.blockProtectionValue;
  ...
  if (state->configuration->statusConfig.blockProtectionWidth != 0U) {
    ...
    status = Qspi_Ip_CheckCommandComplete(
        instance, state->configuration->statusConfig.statusRegReadLut);
    if (STATUS_QSPI_IP_SUCCESS == status) {
      /* Read and check the current setting */
      status = Qspi_Ip_GetProtection(instance, &getProtection);
      if ((STATUS_QSPI_IP_SUCCESS == status) &&
          (getProtection != configProtection)) {
        /* Set new setting */
...
static Qspi_Ip_StatusType Qspi_Ip_InitReset(uint32 instance, uint16 resetCmdLut,
  ...
  if (QSPI_IP_LUT_INVALID != resetCmdLut) {
    for (cnt = 0U; cnt < resetCmdCount; cnt++) {
      ...
      /* Run QSPI command */
      status = Qspi_Ip_IpCommand(state->connection->qspiInstance,
                                 QSPI_IP_COMMAND_LUT, state->baseAddress);
...
static Qspi_Ip_StatusType Qspi_Ip_SerialflashEraseChip(uint32 instance) {
  ...
  if (state->configuration->eraseSettings.chipEraseLut != QSPI_IP_LUT_INVALID) {
    ...
    if (STATUS_QSPI_IP_SUCCESS == status) {
      ...
      status = Qspi_Ip_RunCommand(
          instance, state->configuration->eraseSettings.chipEraseLut, 0U);
    }
  }
...
static Qspi_Ip_StatusType Qspi_Ip_SerialflashGetMemoryStatus(uint32 instance) {
  ...
  status = Qspi_Ip_CheckStatusReg(instance, statusConfig->busyOffset, 1U,
                                  &busyValue);
  /* Check BUSY value */
  if (busyValue == statusConfig->busyValue) {
 ...
 */
static void Qspi_Ip_PatchReadCommand(uint32 instance, uint16 readLut) {
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
...
Qspi_Ip_StatusType Qspi_Ip_RunCommand(uint32 instance, uint16 lut,
                                      uint32 addr) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  ...
  if (QSPI_IP_LUT_INVALID == lut) {
    ...
    /* Run QSPI command */
    status = Qspi_Ip_IpCommand(state->connection->qspiInstance,
                               QSPI_IP_COMMAND_LUT, state->baseAddress + addr);
  }
...
Qspi_Ip_StatusType Qspi_Ip_RunReadCommand(uint32 instance, uint16 lut,
                                          uint32 addr, uint8 *dataRead,
                                          const uint8 *dataCmp, uint32 size) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  ...
  DEV_ASSERT_QSPI(lut != QSPI_IP_LUT_INVALID);
  DEV_ASSERT_QSPI(addr < state->configuration->memSize);
  DEV_ASSERT_QSPI((size > 0UL) &&
                  ((addr + size) <= state->configuration->memSize));
  ...
  if (QSPI_IP_LUT_INVALID == lut) {
    ...
    /* Run QSPI command */
    status =
        Qspi_Ip_IpRead(state->connection->qspiInstance, QSPI_IP_COMMAND_LUT,
                       state->baseAddress + addr, dataRead, dataCmp, size);
  }
...
Qspi_Ip_StatusType Qspi_Ip_RunWriteCommand(uint32 instance, uint16 lut,
                                           uint32 addr, const uint8 *data,
                                           uint32 size) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  ...
  if (QSPI_IP_LUT_INVALID == lut) {
    ...
    /* Run QSPI command */
    status =
        Qspi_Ip_IpWrite(state->connection->qspiInstance, QSPI_IP_COMMAND_LUT,
                        state->baseAddress + addr, data, size);
  }
...
Qspi_Ip_StatusType Qspi_Ip_EraseBlock(uint32 instance, uint32 address,
                                      uint32 size) {
  ...
  DEV_ASSERT_QSPI((size > 0UL) &&
                  ((address + size) <= state->configuration->memSize));
  /* Check address range */
  if (address < state->configuration->memSize) {
    /* find the suited erase type */
    for (eraseType = 0U; eraseType < QSPI_IP_ERASE_TYPES; eraseType++) {
      if ((state->configuration->eraseSettings.eraseTypes[eraseType].eraseLut !=
           QSPI_IP_LUT_INVALID) &&
          (size == (uint32)((uint32)1U << (state->configuration->eraseSettings
    ...
    if (eraseType < QSPI_IP_ERASE_TYPES) {
      status = Qspi_Ip_BasicErase(
          instance, address,
          state->configuration->eraseSettings.eraseTypes[eraseType].eraseLut);
      if (STATUS_QSPI_IP_SUCCESS == status) {
...
Qspi_Ip_StatusType Qspi_Ip_ProgramSuspend(uint32 instance) {
  ...
  status = Qspi_Ip_RunCommand(
      instance, state->configuration->suspendSettings.programSuspendLut, 0U);
...
Qspi_Ip_StatusType Qspi_Ip_ProgramResume(uint32 instance) {
  ...
  status = Qspi_Ip_RunCommand(
      instance, state->configuration->suspendSettings.programResumeLut, 0U);
...
Qspi_Ip_StatusType Qspi_Ip_EraseSuspend(uint32 instance) {
  ...
  status = Qspi_Ip_RunCommand(
      instance, state->configuration->suspendSettings.eraseSuspendLut, 0U);
...
Qspi_Ip_StatusType Qspi_Ip_EraseResume(uint32 instance) {
  ...
  status = Qspi_Ip_RunCommand(
      instance, state->configuration->suspendSettings.eraseResumeLut, 0U);
  state->lastCommand = QSPI_IP_LAST_COMMAND_ERASE;
...
Qspi_Ip_StatusType Qspi_Ip_Reset(uint32 instance) {
  ...
  if (QSPI_IP_LUT_INVALID != resetCmdLut) {
    ...
    if ((STATUS_QSPI_IP_SUCCESS == status) &&
        (state->configuration->ctrlAutoCfgPtr != NULL_PTR)) {
      status = Qspi_Ip_ControllerInit(state->connection->qspiInstance,
                                      state->configuration->ctrlAutoCfgPtr);
    }
    if (STATUS_QSPI_IP_SUCCESS == status) {
...
Qspi_Ip_StatusType Qspi_Ip_GetMemoryStatus(uint32 instance) {
  ...
  if (STATUS_QSPI_IP_SUCCESS == status) {
    ...
    if ((state->lastCommand != QSPI_IP_LAST_COMMAND_NONE) &&
        (NULL_PTR != state->configuration->errorCheckCallout)) {
 ...
 * Description   : Calculates the number of padding bytes to handle reading from
 * unaligned addresses
 */
static inline uint32 Qspi_Ip_CalcPaddingRead(const Qspi_Ip_StateType *state,
                                             uint32 *address, uint32 *size) {
...
Qspi_Ip_StatusType Qspi_Ip_Read(uint32 instance, uint32 address, uint8 *data,
                                uint32 size) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  ...
  DEV_ASSERT_QSPI((crtSize > 0UL) &&
                  ((crtAddress + crtSize) <= state->configuration->memSize));
  DEV_ASSERT_QSPI(instance < QSPI_IP_MEM_INSTANCE_COUNT);
  DEV_ASSERT_QSPI(data != NULL_PTR);
  ...
  /* Prepare timeout counter */
  u32TimeoutTicks = OsIf_MicrosToTicks(QSPI_IP_READ_TIMEOUT,
                                       (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
  u32CurrentTicks = OsIf_GetCounter((OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
  while ((crtSize > 0UL) && (STATUS_QSPI_IP_SUCCESS == status)) {
    ...
    status = Qspi_Ip_RunReadCommand(instance, state->activeReadLut, crtAddress,
                                    crtData, NULL_PTR, chunkSize);
    /* Move to next chunk */
    crtSize -= chunkSize;
    crtAddress += chunkSize;
    crtData = &(
        crtData[chunkSize - offset]); /* Handle alignment for the first read */
    offset = 0U;

    /* Call user callout, if available, to check ecc status */
    if ((STATUS_QSPI_IP_SUCCESS == status) &&
        (NULL_PTR != state->configuration->eccCheckCallout)) {
      status = state->configuration->eccCheckCallout(instance, crtAddress,
                                                     chunkSize);
    }
  }
  return status;
...
Qspi_Ip_StatusType Qspi_Ip_ReadId(uint32 instance, uint8 *data) {
  ...
  /* Check device type */
  if (QSPI_IP_SERIAL_FLASH == state->configuration->memType) {
    status = Qspi_Ip_RunReadCommand(
        instance, state->configuration->readIdSettings.readIdLut, 0U, data,
        NULL_PTR, state->configuration->readIdSettings.readIdSize);
  } else {
    status = Qspi_Ip_HyperflashReadId(
        instance, state->configuration->hfConfig->deviceIdWordAddress, data,
...
Qspi_Ip_StatusType Qspi_Ip_ProgramVerify(uint32 instance, uint32 address,
...
Qspi_Ip_StatusType Qspi_Ip_EraseVerify(uint32 instance, uint32 address,
                                       uint32 size) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  const Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  uint32 crtAddress = address;
 ...
 * Description   : Calculates the number of padding bytes to handle writing to
 * unaligned addresses
 */
static inline uint32 Qspi_Ip_CalcPaddingWrite(const Qspi_Ip_StateType *state,
  ...
  Qspi_Ip_MemoryPadding[qspiInstance] =
      (uint8)((prePadding << 4UL) | postPadding);
...
Qspi_Ip_StatusType Qspi_Ip_Program(uint32 instance, uint32 address,
                                   const uint8 *data, uint32 size) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  Qspi_Ip_StateType *state = &(Qspi_Ip_MemoryStateStructure[instance]);
  ...
  if ((address >= state->configuration->memSize) ||
      ((size > state->configuration->pageSize))) {
    status = STATUS_QSPI_IP_ERROR;
  } else {
    ...
    if (QSPI_IP_SERIAL_FLASH == state->configuration->memType) {
      ...
      if (STATUS_QSPI_IP_SUCCESS == status) {
        ...
        status =
            Qspi_Ip_RunWriteCommand(instance, state->configuration->writeLut,
                                    alignedAddress, data, size);
      }
    } else /*QSPI_IP_HYPER_FLASH == state->configuration->memType */
    {
      /* Perform alignment checking before write data */
...
Qspi_Ip_StatusType Qspi_Ip_Enter0XX(uint32 instance) {
  ...
  if (state->configuration->read0xxLut != QSPI_IP_LUT_INVALID) {
    ...
    status = Qspi_Ip_RunReadCommand(instance, state->activeReadLut, 0U,
                                    &dummyData, NULL_PTR, 1U);
  }

  if (state->configuration->read0xxLutAHB != QSPI_IP_LUT_INVALID) {
    /* Set 0-x-x mode LUT sequence for AHB */
    (void)Qspi_Ip_InitLutSeq(instance, state->configuration->read0xxLutAHB,
                             QSPI_IP_AHB_LUT);
  }
...
Qspi_Ip_StatusType Qspi_Ip_Exit0XX(uint32 instance) {
  ...
  if (state->configuration->read0xxLutAHB != QSPI_IP_LUT_INVALID) {
    ...
    (void)Qspi_Ip_InitLutSeq(instance, state->configuration->readLut,
                             QSPI_IP_AHB_LUT);
  }
...
Qspi_Ip_StatusType Qspi_Ip_SetProtection(uint32 instance, uint8 value) {
  ...
  return Qspi_Ip_UpdateStatusReg(instance, statusConfig->blockProtectionOffset,
                                 statusConfig->blockProtectionWidth, value);
}
...
Qspi_Ip_StatusType Qspi_Ip_GetProtection(uint32 instance, uint8 *value) {
  ...
  return Qspi_Ip_CheckStatusReg(instance, statusConfig->blockProtectionOffset,
                                statusConfig->blockProtectionWidth, value);
}
...
Qspi_Ip_StatusType Qspi_Ip_Init(uint32 instance,
  ...
  state->lastCommand = QSPI_IP_LAST_COMMAND_NONE;
  state->baseAddress =
      Qspi_Ip_GetBaseAdress(pConnect->qspiInstance, pConnect->connectionType);

  /* Initialize corresponding controller if required */
  if (pConfig->ctrlAutoCfgPtr != NULL_PTR) {
    status =
        Qspi_Ip_ControllerInit(pConnect->qspiInstance, pConfig->ctrlAutoCfgPtr);
  }
  if (STATUS_QSPI_IP_SUCCESS == status) {
    /* Perform initial reset */
    status = Qspi_Ip_InitReset(instance, pConfig->initResetSettings.resetCmdLut,
  ...
  /* If enabled, call the init callout, for additional QSPI IP or external
   * memory settings. */
  if ((STATUS_QSPI_IP_SUCCESS == status) &&
      (NULL_PTR != pConfig->initCallout)) {
