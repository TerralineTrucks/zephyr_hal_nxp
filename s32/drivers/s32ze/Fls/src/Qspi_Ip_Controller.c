 ...
 */
#if ((QSPI_IP_CONTROLLER_AR_RELEASE_MAJOR_VERSION_C != \
      MCAL_AR_RELEASE_MAJOR_VERSION) ||                \
     (QSPI_IP_CONTROLLER_AR_RELEASE_MINOR_VERSION_C != \
      MCAL_AR_RELEASE_MINOR_VERSION))
#error \
    "AutoSar Version Numbers of Qspi_Ip_Controller.c and Mcal.h are different"
#endif

/* Check if current file and OsIf.h header file are of the same Autosar version
 */
#if ((QSPI_IP_CONTROLLER_AR_RELEASE_MAJOR_VERSION_C != \
      OSIF_AR_RELEASE_MAJOR_VERSION) ||                \
     (QSPI_IP_CONTROLLER_AR_RELEASE_MINOR_VERSION_C != \
      OSIF_AR_RELEASE_MINOR_VERSION))
#error \
    "AutoSar Version Numbers of Qspi_Ip_Controller.c and OsIf.h are different"
#endif
#endif

/* Check if current file and Qspi_Ip_Controller header file are of the same
 * vendor */
 ...
 * Autosar version */
#if ((QSPI_IP_CONTROLLER_AR_RELEASE_MAJOR_VERSION_C !=    \
      QSPI_IP_CONTROLLER_AR_RELEASE_MAJOR_VERSION_H) ||   \
     (QSPI_IP_CONTROLLER_AR_RELEASE_MINOR_VERSION_C !=    \
      QSPI_IP_CONTROLLER_AR_RELEASE_MINOR_VERSION_H) ||   \
     (QSPI_IP_CONTROLLER_AR_RELEASE_REVISION_VERSION_C != \
      QSPI_IP_CONTROLLER_AR_RELEASE_REVISION_VERSION_H))
#error \
    "AutoSar Version Numbers of Qspi_Ip_Controller.c and Qspi_Ip_Controller.h are different"
#endif
/* Check if current file and Qspi_Ip_Controller header file are of the same
 * Software version */
#if ((QSPI_IP_CONTROLLER_SW_MAJOR_VERSION_C !=  \
      QSPI_IP_CONTROLLER_SW_MAJOR_VERSION_H) || \
     (QSPI_IP_CONTROLLER_SW_MINOR_VERSION_C !=  \
      QSPI_IP_CONTROLLER_SW_MINOR_VERSION_H) || \
     (QSPI_IP_CONTROLLER_SW_PATCH_VERSION_C !=  \
      QSPI_IP_CONTROLLER_SW_PATCH_VERSION_H))
#error \
    "Software Version Numbers of Qspi_Ip_Controller.c and Qspi_Ip_Controller.h are different"
#endif
...
#endif
/* Check if current file and Qspi_Ip_HwAccess header file are of the same
 * Autosar version */
#if ((QSPI_IP_CONTROLLER_AR_RELEASE_MAJOR_VERSION_C !=    \
      QSPI_IP_HW_ACCESS_AR_RELEASE_MAJOR_VERSION_H) ||    \
     (QSPI_IP_CONTROLLER_AR_RELEASE_MINOR_VERSION_C !=    \
      QSPI_IP_HW_ACCESS_AR_RELEASE_MINOR_VERSION_H) ||    \
     (QSPI_IP_CONTROLLER_AR_RELEASE_REVISION_VERSION_C != \
      QSPI_IP_HW_ACCESS_AR_RELEASE_REVISION_VERSION_H))
#error \
    "AutoSar Version Numbers of Qspi_Ip_Controller.c and Qspi_Ip_HwAccess.h are different"
#endif
/* Check if current file and Qspi_Ip_HwAccess header file are of the same
 * Software version */
#if ((QSPI_IP_CONTROLLER_SW_MAJOR_VERSION_C != \
      QSPI_IP_HW_ACCESS_SW_MAJOR_VERSION_H) || \
     (QSPI_IP_CONTROLLER_SW_MINOR_VERSION_C != \
      QSPI_IP_HW_ACCESS_SW_MINOR_VERSION_H) || \
     (QSPI_IP_CONTROLLER_SW_PATCH_VERSION_C != \
      QSPI_IP_HW_ACCESS_SW_PATCH_VERSION_H))
#error \
    "Software Version Numbers of Qspi_Ip_Controller.c and Qspi_Ip_HwAccess.h are different"
#endif
 ...
 */
#if ((QSPI_IP_CONTROLLER_AR_RELEASE_MAJOR_VERSION_C !=    \
      QSPI_IP_AR_RELEASE_MAJOR_VERSION_H) ||              \
     (QSPI_IP_CONTROLLER_AR_RELEASE_MINOR_VERSION_C !=    \
      QSPI_IP_AR_RELEASE_MINOR_VERSION_H) ||              \
     (QSPI_IP_CONTROLLER_AR_RELEASE_REVISION_VERSION_C != \
      QSPI_IP_AR_RELEASE_REVISION_VERSION_H))
#error \
    "AutoSar Version Numbers of Qspi_Ip_Controller.c and Qspi_Ip.h are different"
#endif
/* Check if current file and Qspi_Ip header file are of the same Software
 * version */
...
#error \
    "Software Version Numbers of Qspi_Ip_Controller.c and Qspi_Ip.h are different"
#endif
...
#define QuadSPI_SOCCR_PD(x) \
  (((uint32)(((uint32)(x)) << QuadSPI_SOCCR_PD_SHIFT)) & QuadSPI_SOCCR_PD_MASK)
...
#define Qspi_Ip_Sfp_Configure(baseAddr, userConfigPtr)                 \
  OsIf_Trusted_Call2params(Qspi_Ip_Sfp_Configure_Privileged, baseAddr, \
                           userConfigPtr)
#define Qspi_Ip_ResetPrivilegedRegisters(baseAddr) \
  OsIf_Trusted_Call1param(Qspi_Ip_ResetPrivilegedRegisters_Privileged, baseAddr)
#define Qspi_Ip_Sfp_ClearLatchedErrors(baseAddr) \
  OsIf_Trusted_Call1param(Qspi_Ip_Sfp_ClearLatchedErrors_Privileged, baseAddr)

#else /* QSPI_IP_ENABLE_USER_MODE_SUPPORT */

#define Qspi_Ip_Sfp_Configure(baseAddr, userConfigPtr) \
  Qspi_Ip_Sfp_Configure_Privileged(baseAddr, userConfigPtr)
#define Qspi_Ip_ResetPrivilegedRegisters(baseAddr) \
  Qspi_Ip_ResetPrivilegedRegisters_Privileged(baseAddr)
#define Qspi_Ip_Sfp_ClearLatchedErrors(baseAddr) \
  Qspi_Ip_Sfp_ClearLatchedErrors_Privileged(baseAddr)
...
#define QSPI_ERR_FLAGS_MASK                                               \
  (QuadSPI_FR_TBUF_MASK | QuadSPI_FR_ILLINE_MASK | QuadSPI_FR_RBOF_MASK | \
   QuadSPI_FR_IPAEF_MASK | QuadSPI_FR_IPIEF_MASK)

#define FLS_START_SEC_CONST_UNSPECIFIED
#include "Fls_MemMap.h"

/* Table of base addresses for QuadSPI instances. */
QuadSPI_Type *const Qspi_Ip_BaseAddress[QuadSPI_INSTANCE_COUNT] =
    IP_QuadSPI_BASE_PTRS;
...
/* The padding bytes information to handle unaligned read/write operation for
   QuadSPI instances:
   - For read:
        [7:0] the number of padding bytes to handle read from unaligned address
   - For write:
        [7:4] the number of pre-padding bytes to handle write from unaligned
   start address [3:0] the number of post-padding bytes to handle write from
...
/* Pointer to runtime configuration structures (storing the DLL configuration
 * for runtime usage) */
const Qspi_Ip_ControllerConfigType
    *Qspi_Ip_ControllerConfig[QuadSPI_INSTANCE_COUNT];
...
void Qspi_Ip_Sfp_Configure_Privileged(
    QuadSPI_Type *baseAddr, Qspi_Ip_ControllerConfigType const *userConfigPtr);
...
static inline Qspi_Ip_StatusType Qspi_Ip_ProcessDataRead(
    uint8 *dataRead, uint32 size, const QuadSPI_Type *baseAddr,
    uint32 padding) {
  uint8 *data = dataRead;
  uint32 cnt = 0U;
  uint32 recvData;
  ...
  if (((Qspi_Ip_UintPtrType)data & 0x3U) == 0U) {
    ...
    while (sizeRemaining >= 4U) {
      *((uint32 *)((Qspi_Ip_UintPtrType)data)) =
          baseAddr->RBDR[cnt]; /* Casting through uint32 to avoid Misra 11.3 */
      data = &(data[4U]);
      cnt++;
      sizeRemaining -= 4U;
    }
...
static inline Qspi_Ip_StatusType Qspi_Ip_ProcessDataVerify(
    const uint8 *dataCmp, uint32 size, const QuadSPI_Type *baseAddr,
    uint32 padding) {
  ...
  if (((Qspi_Ip_UintPtrType)roData & 0x3U) == 0U) {
    while (sizeRemaining >= 4U) {
      /* Process 4 bytes at a time to speed up read */
      if (*((const uint32 *)((Qspi_Ip_UintPtrType)roData)) !=
          baseAddr->RBDR[cnt]) {
        /* return STATUS_QSPI_IP_ERROR_PROGRAM_VERIFY if the data is not match
         */
        status = STATUS_QSPI_IP_ERROR_PROGRAM_VERIFY;
...
static inline Qspi_Ip_StatusType Qspi_Ip_ProcessDataBlankCheck(
    uint32 size, const QuadSPI_Type *baseAddr, uint32 padding) {
  uint32 cnt = 0U;
  uint32 recvData = 0U;
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  uint32 dataSize;
...
#if (defined(CORE_BIG_ENDIAN))
    if ((~recvData &
         ~(((uint32)1U << (((uint32)4U - sizeRemaining) * 8U)) - 1U)) != 0U)
#else
...
static void Qspi_Ip_FillTxBuf(QuadSPI_Type *baseAddr, const uint8 *roData,
                              uint32 size, uint32 paddingInfo) {
  ...
  if (prePadding != 0U) {
    ...
    if (wordSize > 4U) {
      ...
      /* Note for special case: prePadding + Data + postPadding are fit into a
         word Decreaseing postPadding is not needed because out of user data,
         all branches below will be skipped
       */
      sizeLeft = 0U;
    }
  ...
  if (((Qspi_Ip_UintPtrType)roDataPtr & 0x3U) == 0U) {
    ...
    while (sizeLeft >= 4U) {
      data = *(const uint32 *)((
          Qspi_Ip_UintPtrType)roDataPtr); /* Casting through uint32 to avoid
                                             Misra 11.3 */
      sizeLeft -= 4U;
...
static void Qspi_Ip_AhbFlush(QuadSPI_Type *baseAddr) {
  ...
  /* Prepare timeout counter */
  u32TimeoutTicks = OsIf_MicrosToTicks(QSPI_IP_CMD_COMPLETE_TIMEOUT,
                                       (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
  ...
  while (!Qspi_Ip_GetClrAhbStatus(baseAddr)) {
    /* An exit point for safety purpose only, because this loop is not expected
     * to happen in practice */
    u32ElapsedTicks += OsIf_GetElapsed(&u32CurrentTicks,
                                       (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
 ...
 * Description   : DLL side A - Wait for slave high lock status or DLL lock
 * status
 */
static Qspi_Ip_StatusType Qspi_Ip_WaitDLLASlaveLock(
    const QuadSPI_Type *baseAddr, boolean waitSlaveLock) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
  boolean LockStatus;

  /* Prepare timeout counter */
  uint32 u32ElapsedTicks = 0UL;
  uint32 u32TimeoutTicks = OsIf_MicrosToTicks(
      QSPI_IP_DLL_LOCK_TIMEOUT, (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
  uint32 u32CurrentTicks =
      OsIf_GetCounter((OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);

  do {
    /* Wait for slave high lock status or DLL lock status */
    LockStatus = (waitSlaveLock) ? Qspi_Ip_DLLGetSlaveLockStatusA(baseAddr)
                                 : Qspi_Ip_DLLGetLockStatusA(baseAddr);
    ...
    if (Qspi_Ip_DLLGetErrorStatusA(baseAddr)) {
      ...
      /* Check for timeout */
      u32ElapsedTicks += OsIf_GetElapsed(
          &u32CurrentTicks, (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
      if (u32ElapsedTicks >= u32TimeoutTicks) {
 ...
 */
static Qspi_Ip_StatusType Qspi_Ip_ConfigureDLLAByPass(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
 ...
 */
static Qspi_Ip_StatusType Qspi_Ip_ConfigureDLLAUpdate(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
...
#ifdef QuadSPI_DLLCRA_SLAVE_AUTO_UPDT_MASK
  Qspi_Ip_DLLSlaveAutoUpdateA(baseAddr, (QSPI_IP_DLL_AUTO_UPDATE ==
                                         userConfigPtr->dllSettingsA.dllMode));
#endif /* QuadSPI_DLLCRA_SLAVE_AUTO_UPDT_MASK */
  Qspi_Ip_DLLSetReferenceCounterA(baseAddr,
                                  userConfigPtr->dllSettingsA.referenceCounter);
  ...
  Qspi_Ip_DLLSetCoarseOffsetA(baseAddr,
                              userConfigPtr->dllSettingsA.coarseDelay);
  Qspi_Ip_DLLSetFineOffsetA(baseAddr, userConfigPtr->dllSettingsA.fineDelay);
  Qspi_Ip_DLLFreqEnA(baseAddr, userConfigPtr->dllSettingsA.freqEnable);
  ...
  if (QSPI_IP_DLL_MANUAL_UPDATE == userConfigPtr->dllSettingsA.dllMode) {
    /* For manual update mode, wait for DLL lock before triggering slave chain
     * update */
...
static Qspi_Ip_StatusType Qspi_Ip_ConfigureDLLA(
    uint32 instance, QuadSPI_Type *baseAddr,
 ...
 *
 *END**************************************************************************/
static Qspi_Ip_StatusType Qspi_Ip_ConfigureDLL(
    uint32 instance, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_SUCCESS;
 ...
 ***********************************************************************/
#if defined(FEATURE_QSPI_CHIP_OPTIONS_S32K148)
static void Qspi_Ip_ConfigureChipOptions(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
...
static void Qspi_Ip_ConfigureChipOptions(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  (void)userConfigPtr;
  Qspi_Ip_SetChipOptions(baseAddr,
                         0x0000000E); /* set ibe=1, obe=1, dse=1 and sre=0 */
}
...
static void Qspi_Ip_ConfigureReadOptions(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  /* Always enable DQS */
...
static Qspi_Ip_StatusType Qspi_Ip_AhbSetup(
    uint32 instance, const Qspi_Ip_ControllerAhbConfigType *config) {
  QuadSPI_Type *baseAddr;

  DEV_ASSERT_QSPI(instance < QuadSPI_INSTANCE_COUNT);
  DEV_ASSERT_QSPI(0U == (config->sizes[0U] & 7U));
  DEV_ASSERT_QSPI(((uint32)config->sizes[0U] + (uint32)config->sizes[1U] +
                   (uint32)config->sizes[2U] + (uint32)config->sizes[3U]) <=
                  FEATURE_QSPI_AHB_BUF_SIZE);
  ...
  /* Set AHB buffer 3 */
  Qspi_Ip_SetAhbBuf3(baseAddr, config->sizes[3U], config->masters[3U],
                     config->allMasters);
  /* Set AHB buffer index 0 */
  Qspi_Ip_SetAhbBuf0Ind(baseAddr, (uint32)config->sizes[0U]);
  /* Set AHB buffer index 1 */
  Qspi_Ip_SetAhbBuf1Ind(baseAddr,
                        (uint32)config->sizes[0U] + (uint32)config->sizes[1U]);
  /* Set AHB buffer index 2 */
  Qspi_Ip_SetAhbBuf2Ind(baseAddr, (uint32)config->sizes[0U] +
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * * * * * * * * * */
static void Qspi_Ip_NewIpsTransaction(QuadSPI_Type *BaseAddress, uint32 Address,
                                      uint16 DataSize, uint8 SeqId) {
#if (FEATURE_QSPI_HAS_SFP == 1)
...
static Qspi_Ip_StatusType Qspi_Ip_Sfp_WaitFsmState(
    QuadSPI_Type const *BaseAddress, uint32 State) {
  ...
  uint32 TimeoutTicks =
      OsIf_MicrosToTicks(QSPI_IP_QSPI_IDLE_TIMEOUT, QSPI_IP_TIMEOUT_TYPE);
  ;
  ...
  do {
    ...
    FsmStatValid =
        (FsmStat & QuadSPI_FSMSTAT_VLD_MASK) >> QuadSPI_FSMSTAT_VLD_SHIFT;
    FsmStatState =
        (FsmStat & QuadSPI_FSMSTAT_STATE_MASK) >> QuadSPI_FSMSTAT_STATE_SHIFT;
 ...
 * Description   : Configures a pair of LUT commands in the specified LUT
 *register
 *
 *END**************************************************************************/
void Qspi_Ip_SetLut(uint32 instance, uint8 LutRegister,
 ...
 * Description   : Configures pairs of LUT commands from the specified LUT
 *register
 *
 *END**************************************************************************/
void Qspi_Ip_WriteLuts_Privileged(uint32 Instance, uint8 StartLutRegister,
                                  const uint32 *Data, uint8 Size) {
  DEV_ASSERT_QSPI(Instance < QuadSPI_INSTANCE_COUNT);
 ...
 *
 *END**************************************************************************/
static inline void Qspi_Ip_SetSerialFlashAddress(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  Qspi_Ip_SetAddrOptions(baseAddr, userConfigPtr->columnAddr,
                         userConfigPtr->wordAddresable);
 ...
 *
 *END**************************************************************************/
static inline void Qspi_Ip_SetCsTime(
    QuadSPI_Type *baseAddr, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
 ...
 *
 *END**************************************************************************/
static inline void Qspi_Ip_ConfigureBuffers(
    uint32 instance, QuadSPI_Type *baseAddr,
    const Qspi_Ip_ControllerConfigType *userConfigPtr) {
 ...
 *
 *END**************************************************************************/
static inline void Qspi_Ip_ConfigureControllerA(
    uint32 instance, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  QuadSPI_Type *baseAddr = Qspi_Ip_BaseAddress[instance];

  /* Configure external flash memory map Size A */
  Qspi_Ip_SetMemMapSizeA(instance, baseAddr, userConfigPtr->memSizeA1,
                         userConfigPtr->memSizeA2);

  /* Configure idle line value side A */
  Qspi_Ip_SetIdleLineValuesA(baseAddr, userConfigPtr->io2IdleValueA,
                             userConfigPtr->io3IdleValueA);
}

/*FUNCTION**********************************************************************
 *
 * Function Name : Qspi_Ip_ConfigureController
 * Description   : Configure the controller register following user
 *configurations
 *
 *END**************************************************************************/
static inline void Qspi_Ip_ConfigureController(
    uint32 instance, const Qspi_Ip_ControllerConfigType *userConfigPtr) {
  QuadSPI_Type *baseAddr = Qspi_Ip_BaseAddress[instance];
  ...
  Qspi_Ip_SetRxCfg(baseAddr, userConfigPtr->sampleDelay,
                   userConfigPtr->samplePhase);
 ...
 * Description   : Force the Qspi controller to cancel any on-going transactions
 * by performing the software reset requence. This can be used to recover the
 * hardware controller from being stuck in BUSY state. Re-configure DLL is
 * needed to avoid side-effects on the DLL after the S/W reset.
 */
Qspi_Ip_StatusType Qspi_Ip_Abort(uint32 instance) {
  ...
  /* S/W reset would also loose any Slave-chain upate, therefore re-configure
   * DLL sequence is needed */
  if (Qspi_Ip_ControllerConfig[instance] != NULL_PTR) {
    /* Perform the DLL config only if the user configuration is available.
       Because there will be the case when user skip the controller
 ...
 * Description   : Returns the physical base address of a flash device on the
 * AHB bus. The controller must be initialized prior to calling this function.
...
Qspi_Ip_StatusType Qspi_Ip_IpCommand(uint32 instance, uint8 SeqId,
                                     uint32 addr) {
  QuadSPI_Type *baseAddr;
  Qspi_Ip_StatusType status;
  ...
  /* Make sure there is no garbage in Rx fifo in case of triggering a dummy READ
     instruction. This clears RBSR[RDBFL] to ensure QuadSPI is idle from SFP
     point of view and avoid the Master timeout error.
   */
...
Qspi_Ip_StatusType Qspi_Ip_IpRead(uint32 instance, uint8 SeqId, uint32 addr,
                                  uint8 *dataRead, const uint8 *dataCmp,
                                  uint32 size) {
  Qspi_Ip_StatusType status = STATUS_QSPI_IP_ERROR;
  QuadSPI_Type *baseAddr;
  uint32 padding;
 ...
 * Description   : Invalidates the TX buffer content and wait until it is
...
Qspi_Ip_StatusType Qspi_Ip_IpWrite(uint32 instance, uint8 SeqId, uint32 addr,
  ...
  Qspi_Ip_SetTxWatermark(baseAddr, (uint8)((FEATURE_QSPI_TX_BUF_SIZE / 4U) -
                                           ((TotalSize / 4U) - 1U)));

  /* 01 - TBDR lock is open. QuadSPI considers IPS transfer. Master counter is
   * started. */
  if (STATUS_QSPI_IP_SUCCESS == Qspi_Ip_Sfp_WaitFsmState(baseAddr, 1U)) {
    ...
    /* 10 - TX buffer filled above threshold. Write transfer is triggered. SEQID
     * is written. */
    if (((baseAddr->FSMSTAT & QuadSPI_FSMSTAT_STATE_MASK) >>
         QuadSPI_FSMSTAT_STATE_SHIFT) == 2U) {
 ...
 */
static void Qspi_Ip_Sfp_Configure_Mdad(
    QuadSPI_Type *baseAddr, Qspi_Ip_ControllerConfigType const *userConfigPtr) {
  for (uint8 Index = 0U; Index < QuadSPI_MDAD_COUNT; ++Index) {
    if (TRUE == userConfigPtr->SfpCfg.Tg[Index].Valid) {
      Qspi_Ip_Sfp_SetTgSecureAttribute(
          baseAddr, Index, userConfigPtr->SfpCfg.Tg[Index].SecureAttribute);
      Qspi_Ip_Sfp_SetTgMaskType(baseAddr, Index,
                                userConfigPtr->SfpCfg.Tg[Index].MaskType);
      Qspi_Ip_Sfp_SetTgMask(baseAddr, Index,
                            userConfigPtr->SfpCfg.Tg[Index].Mask);
      Qspi_Ip_Sfp_SetTgDomainIdMatch(baseAddr, Index,
                                     userConfigPtr->SfpCfg.Tg[Index].DomainId);
      Qspi_Ip_Sfp_SetTgValid(baseAddr, Index,
                             userConfigPtr->SfpCfg.Tg[Index].Valid);
    }
  }
}
 ...
 */
static void Qspi_Ip_Sfp_Configure_Frad(
    QuadSPI_Type *baseAddr, Qspi_Ip_ControllerConfigType const *userConfigPtr) {
  for (uint8 Index = 0U; Index < QuadSPI_FRAD_COUNT; ++Index) {
    if (TRUE == userConfigPtr->SfpCfg.Frad[Index].Valid) {
      Qspi_Ip_Sfp_SetFradStartAddress(
          baseAddr, Index, userConfigPtr->SfpCfg.Frad[Index].StartAddress);
      Qspi_Ip_Sfp_SetFradEndAddress(
          baseAddr, Index, userConfigPtr->SfpCfg.Frad[Index].EndAddress);
      Qspi_Ip_Sfp_SetFradMd0Acp(
          baseAddr, Index, (uint8)userConfigPtr->SfpCfg.Frad[Index].Md0Acp);
      Qspi_Ip_Sfp_SetFradMd1Acp(
          baseAddr, Index, (uint8)userConfigPtr->SfpCfg.Frad[Index].Md1Acp);
      Qspi_Ip_Sfp_SetFradEaLock(
          baseAddr, Index,
          userConfigPtr->SfpCfg.Frad[Index].ExclusiveAccessLock);
      Qspi_Ip_Sfp_SetFradEaOwner(
          baseAddr, Index,
          userConfigPtr->SfpCfg.Frad[Index].ExclusiveAccessOwner);
      Qspi_Ip_Sfp_SetFradValid(baseAddr, Index,
                               userConfigPtr->SfpCfg.Frad[Index].Valid);
    }
  }
}
...
void Qspi_Ip_Sfp_Configure_Privileged(
    QuadSPI_Type *baseAddr, Qspi_Ip_ControllerConfigType const *userConfigPtr) {
  /* Workaround for K396: QSPI must be enabled in order to write into SFP
   * registers */
  /* Enable QuadSPI module */
  Qspi_Ip_Enable(baseAddr);

  Qspi_Ip_Sfp_SetAccessControls(baseAddr, QSPI_IP_SFP_ALL,
                                QSPI_IP_SFP_ENABLE_GLOBAL);
  Qspi_Ip_Sfp_SetAccessControls(baseAddr, QSPI_IP_SFP_MDAD,
                                QSPI_IP_SFP_ENABLE_MDAD);
  Qspi_Ip_Sfp_SetAccessControls(baseAddr, QSPI_IP_SFP_FRAD,
...
inline static void Qspi_Ip_ResetAllRegisters(QuadSPI_Type *BaseAddr) {
  ...
  BaseAddr->SPTRCLR = (uint32)0x01000000UL |
                      (uint32)QuadSPI_SPTRCLR_BFPTRC_MASK |
                      (uint32)QuadSPI_SPTRCLR_IPPTRC_MASK;
  ...
  /* reset SPTRCLR register: clear both IP and AHB sequence pointers */
  BaseAddr->SPTRCLR =
      (uint32)QuadSPI_SPTRCLR_BFPTRC_MASK | (uint32)QuadSPI_SPTRCLR_IPPTRC_MASK;
  /* reset SFA1AD register */
  BaseAddr->SFA1AD = (uint32)0x70000000UL;
  /* reset SFA2AD register */
  BaseAddr->SFA2AD = (uint32)0x70000000UL;
